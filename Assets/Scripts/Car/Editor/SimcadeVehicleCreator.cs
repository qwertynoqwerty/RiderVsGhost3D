...
namespace Ashsvp
{
    public class SimcadeVehicleCreator : EditorWindow
    {
        ...
        private void createVehicle()
        {
            ...
            {
                DestroyImmediate(NewVehicle.transform.Find("Wheels").Find("wheel FL").Find("wheel mesh FL").GetChild(0).gameObject);
            }
            if (NewVehicle.transform.Find("Wheels").Find("wheel FR"))
            {
                DestroyImmediate(NewVehicle.transform.Find("Wheels").Find("wheel FR").Find("wheel mesh FR").GetChild(0).gameObject);
            }
            if (NewVehicle.transform.Find("Wheels").Find("wheel RL"))
            {
                DestroyImmediate(NewVehicle.transform.Find("Wheels").Find("wheel RL").Find("wheel mesh RL").GetChild(0).gameObject);
            }
            if (NewVehicle.transform.Find("Wheels").Find("wheel RR"))
            {
                DestroyImmediate(NewVehicle.transform.Find("Wheels").Find("wheel RR").Find("wheel mesh RR").GetChild(0).gameObject);
            }
